 ...
 * note:审核意见填写dialog
 */
public class FileDownloadApplyDialog extends Dialog {

    private OnClickRateDialog onClickRateListener;
    private EditText et_opinion;

    public FileDownloadApplyDialog(Context context) {
        super(context);
        setCustomDialog();
    }

    private void setCustomDialog() {
        View mView = LayoutInflater.from(getContext()).inflate(R.layout.dialog_file_download_apply, null);
        ...
        rightButton.setOnClickListener(v -> {
            if (onClickRateListener != null) {
                if (TextUtils.isEmpty(et_opinion.getText().toString().trim())) {
                    ToastUtil.show(getContext(), "请填写申请用途");
                    return;
                }
